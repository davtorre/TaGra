#!/usr/bin/env python
"""
Sepsis / SIRS – preprocessing pipeline.

Loads the CSV, applies column-typed preprocessing, and saves a bundle to bundles/.

Output:
    bundles/sepsis_bundle.npz   – arrays X, X_raw, true_labels
    bundles/sepsis_meta.json    – feat_names, gower_ftypes, n, class_balance,
                                   dataset_name

Usage:
    cd dev/tasks/clustering
    python3 preprocess_sepsis.py
"""
...
from preprocess_typed import (
    ...
    SEPSIS_SPEC, SEPSIS_IMPUTE,
)


def load_sepsis() -> pd.DataFrame:
    df = pd.read_csv(DATA_PATH)
    df["target_binary"] = df["Mortality"].astype(int)
    return df


def prepare_raw_for_gower(df: pd.DataFrame):
    df2 = df.copy()
    for col, strat in SEPSIS_IMPUTE.items():
        if col not in df2.columns:
            continue
        df2[col] = df2[col].fillna(
            df2[col].median() if strat == "median" else df2[col].mode()[0]
        )
    feat_cols = [c for c, t in SEPSIS_SPEC.items()
...
def main():
    ...
    X, feat_names = preprocess_typed(df, SEPSIS_SPEC, SEPSIS_IMPUTE, final_minmax=True)
    ...
    gower_ftypes    = feature_type_map_for_gower(SEPSIS_SPEC, df)
    class_balance   = {str(k): int(v)
                       for k, v in zip(*np.unique(true_labels, return_counts=True))}

    bundle_path = os.path.join(BUNDLE_DIR, "sepsis_bundle.npz")
    meta_path   = os.path.join(BUNDLE_DIR, "sepsis_meta.json")
