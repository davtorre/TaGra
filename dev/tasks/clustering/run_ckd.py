#!/usr/bin/env python
"""
CKD Chronic Kidney Disease – clustering comparison pipeline.
...
Target variable (binary):
    0 = notckd (healthy, 150 samples)
    1 = ckd    (chronic kidney disease, 250 samples)
...
DATA_PATH  = os.path.join(REPO_ROOT,
    "examples/datasets/UCI/Chronic_Kidney_Disease/chronic_kidney_disease_full.arff")
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "clustering_results_ckd")

# ── Data loading ──────────────────────────────────────────────────────────────

def load_ckd():
    """Manual ARFF parser (handles whitespace-padded values)."""
    col_names, rows = [], []
    in_data = False
    ...
    # Numeric columns
    num_ord = ["age","bp","sg","al","su","bgr","bu","sc","sod","pot",
               "hemo","pcv","wbcc","rbcc"]
    for c in num_ord:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    # Binary encoding: 1 = positive / normal / present / good
...
def prepare_raw_for_gower(df):
    """Return unscaled feature matrix for GowerDistanceConstructor."""
    df2 = df.copy()
    for col, strat in CKD_IMPUTE.items():
...
def main(cfg: dict = None):
    ...
    print("=" * 78)
    print("CKD Chronic Kidney Disease – Clustering Comparison Pipeline")
    print(f"Generated: {datetime.now():%Y-%m-%d %H:%M:%S}")
    print("=" * 78)

    # ── Step 1: dataset review ────────────────────────────────────────────
    print("\n── Step 1: Dataset review ──")
    print(f"  Samples: {n}  |  Features: {len(CKD_SPEC)-1}")
    print(f"  Class distribution:\n{df['class'].value_counts().to_string()}")
    print(f"\n  Binary target: 0=notckd {(df['target_binary']==0).sum()}  "
          f"1=ckd {(df['target_binary']==1).sum()}")
    miss_cols = [c for c in CKD_IMPUTE if c in df.columns]
    miss = df[miss_cols].isnull().sum()
    ...
    print("\n── Step 2: DBSCAN fine sweep (eps=0.5–2.0, ms=3–30) ──")
    print(f"  {'eps':>5} {'ms':>4} {'Cl':>4} {'Noise%':>7} {'DBCV':>8} {'Sil':>8} {'Vm':>8}")
    print("  " + "-"*55)

    eps_grid = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.7, 2.0]
    ...
    best_eps = best_eps_candidates[0][0] if best_eps_candidates else 1.1
    ...
    #   DBSCAN Ref A : eps=0.5  ms=10  → 3cl, 57.4% noise, DBCV=0.709, Vm=0.775
    #   DBSCAN Ref B : eps=0.6  ms=15  → 2cl, 59.4% noise, DBCV=0.517, Vm=0.700
    #   Gower/range  : t=0.05 df=6    → 3cl, 58.9% noise, DBCV=0.732, Vm=0.802
    #   Gower/gauss  : t=0.07 df=2    → 5cl, 59.4% noise, DBCV=0.715, Vm=0.750
    #   TaGra/Dist   : t=0.6  df=6    → 3cl, 57.6% noise, DBCV=0.719, Vm=0.773
    #   TaGra/Sim    : t=0.95 df=9    → 3cl, 57.4% noise, DBCV=0.522, Vm=0.758

    _default_curated = [
        ("dbscan",        {"eps": 0.5,  "ms": 10}),
        ("dbscan",        {"eps": 0.6,  "ms": 15}),
        ("dbscan_constr", {"eps": 0.6,  "ms": 15}),
        ("hdbscan",       {"mcs": 15}),
        ("gower",         {"metric": "range",    "t": 0.05, "df": 6}),
        ("gower",         {"metric": "gaussian", "t": 0.07, "df": 2}),
        ("dist",          {"t": 0.6, "df": 6}),
        ("sim",           {"t": 0.95, "df": 9}),
    ]
    ...
    curated_rows = []
    ref_lbl_A = DBSCAN(eps=0.5, min_samples=10).fit_predict(X)
    ...
    curated_title = ("CURATED COMPARISON – best selected configs\n"
                     "  Vm(ref) = Vm vs DBSCAN Ref A (eps=0.5, ms=10)")
    ...
    # ── Save ───────────────────────────────────────────────────────────────
    report_path = os.path.join(OUTPUT_DIR, "ckd_report.txt")
    with open(report_path, "w") as f:
        def fw(s=""):
            f.write(s + "\n")

        fw("CKD Chronic Kidney Disease – Clustering Comparison")
...
if __name__ == "__main__":
    _default_cfg = os.path.join(os.path.dirname(__file__), "config_ckd.json")
    parser = argparse.ArgumentParser(description="CKD clustering comparison")
    parser.add_argument("--config", default=_default_cfg,
                        help="Path to JSON config file (default: config_ckd.json)")
